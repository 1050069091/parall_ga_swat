...
my_varition_rate_in_file = variation_rate !* (1-cur_num/(recur_rank+1))
...
if(random_real_val <= my_varition_rate_in_file) then
    ...
    outter:do i=1,count_ga
        ...
        tmp_real = real_para_values_arr(varition_para_index) +&
            random_real_val*(random_real_val-0.5)*&
        ...
        if(tmp_real >= min_para_arr(para_index)&
            ...
            real_para_values_arr(varition_para_index) = &
                random_real_val*(max_para_arr(para_index)-min_para_arr(para_index))
