...
if(myid == numprocs-1) then
    ...
    call system('find ./out -name "*.'//best_sim_myid_str//'" | awk -F "." '//"'"&
        //'{print "."$2"'//'."$3}'//"' | xargs -i mv -f {}."//myid_str//" {}")
    ...
    write(*,'(/)') 
    if((g_best_int_obj_fn_val)/10000.0 < obj_fn_threshold) then
        write(*,'(" best object function value(",F8.4,") not arrive the threshold(",F4.2,")")') &
                (g_best_int_obj_fn_val)/10000.0, obj_fn_threshold
    else    
